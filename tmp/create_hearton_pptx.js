...
const REVIEW_COUNT = "140件（6サイト）";
const REPORT_DATE = "2026年3月28日";
const OUTPUT_DIR = "納品レポート/ホテル別レポート";

// Slide 2: KPI
const KPI_AVG = "8.59";
const KPI_HIGH_RATE = "82.9%";
const KPI_LOW_RATE = "1.4%";
const KPI_TOTAL_COUNT = "140件";

// Slide 2: 強み
const SUMMARY_STRENGTHS = [
  { title: "立地・アクセス", desc: "駅近・商業施設近接・76件言及。最強の差別化ポイント" },
  { title: "客室・設備の充実", desc: "55件言及。広い浴室・清潔な客室・快適なベッドが好評" },
  { title: "清潔感", desc: "38件言及。清潔に保たれた空間がゲストの信頼を獲得" },
];

// Slide 2: 弱み
const SUMMARY_WEAKNESSES = [
  { title: "Google評価の低さ", desc: "平均6.8点（要改善）。他サイト比較で2点以上の乖離" },
  { title: "スタッフ対応の不均質", desc: "一部フロントスタッフへのクレームが低評価（2点）の主因" },
  { title: "バスルームの臭い", desc: "生乾き臭が3件言及。換気・配管の改善が必要" },
];

// Slide 3: サイト別
const SITE_CHART_LABELS = ["Trip.com", "じゃらん", "楽天トラベル", "Agoda", "Booking.com", "Google"];
const SITE_CHART_VALUES = [9.43, 8.63, 8.58, 8.55, 8.50, 6.80];

const SITE_INSIGHT_TEXTS = [
  { text: "サイト別インサイト", options: { bold: true, fontSize: 12, breakLine: true } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "Trip.comが突出した高評価", options: { bold: true, color: "16A34A", breakLine: true } },
  { text: "9.43点でインバウンド市場での強い競争力を示す", options: { fontSize: 9, color: "64748B", breakLine: true } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "Googleのみ要改善（6.80点）", options: { bold: true, color: "DC2626", breakLine: true } },
  { text: "他5サイトは全て8.5点以上で安定。Google評価の底上げが最重要課題", options: { fontSize: 9, color: "64748B", breakLine: true } },
  { text: "", options: { fontSize: 6, breakLine: true } },
...
const SITE_TABLE_ROWS_DATA = [
  ["Trip.com", "23", "9.43", "/10", "9.43"],
  ["じゃらん", "41", "4.32", "/5", "8.63"],
  ["楽天トラベル", "31", "4.29", "/5", "8.58"],
  ["Agoda", "11", "8.55", "/10", "8.55"],
  ["Booking.com", "24", "8.50", "/10", "8.50"],
  ["Google", "10", "3.40", "/5", "6.80"],
];

// Slide 4: 評価分布
const DIST_DOUGHNUT_LABELS = ["高評価(8-10)", "中評価(5-7)", "低評価(1-4)"];
const DIST_DOUGHNUT_VALUES = [116, 22, 2];

const DIST_BAR_LABELS = ["10点", "9点", "8点", "7点", "6点", "2点"];
const DIST_BAR_VALUES = [63, 7, 46, 6, 16, 2];

const DIST_SUMMARY_CARDS = [
  { label: "高評価 8-10点", val: "116件（82.9%）", col: "16A34A", bg: "DCFCE7" },
  { label: "中評価 5-7点", val: "22件（15.7%）", col: "D4A843", bg: "FFF7ED" },
  { label: "低評価 1-4点", val: "2件（1.4%）", col: "EA580C", bg: "FEF3C7" },
];

// Slide 5: 強み分析
const STRENGTHS_CARDS = [
  { theme: "立地・アクセス", count: "76件", desc: "駅近・イオン等商業施設近接・チェックアウト12時対応", quote: "「駅近で徒歩圏内に飲食店やスーパーもあり、アクセスもいい」" },
  { theme: "客室・設備", count: "55件", desc: "広い浴室・清潔な客室・快適なベッドが評価の核心", quote: "「お風呂も広いし 部屋も綺麗 アクセスもいいし、また利用したいです！」" },
  { theme: "清潔感", count: "38件", desc: "清潔に保たれた空間がゲストの信頼基盤を形成", quote: "「ホテルは清潔そのもので、スタッフの方々もとても親切です」" },
  { theme: "スタッフ対応", count: "22件", desc: "フロントの丁寧な対応・客室アップグレード対応が好評", quote: "「スタッフの方達は、対応がとても良かったです」" },
  { theme: "朝食", count: "19件", desc: "リーズナブルな朝食付きプランの評価が高い", quote: "「朝食付きで料金も非常にリーズナブルだったと思います」" },
  { theme: "コスパ・リピート", count: "6件", desc: "高コスパとリピート意向を示すコメントが見られる", quote: "「とても居心地が良くて最高でした！また泊まりたいです！」" },
];

// Slide 6: 弱み
const WEAKNESS_ITEMS = [
  { pri: "S", cat: "Google評価低下", detail: "平均6.8点（要改善）。他サイトと2点以上の乖離。国内ゲストの不満が反映", count: "10件", color: "DC2626" },
  { pri: "A", cat: "スタッフ接客不均質", detail: "特定フロントスタッフへの対応クレームが低評価（2点）の主因", count: "2件", color: "EA580C" },
  { pri: "A", cat: "バスルームの臭い", detail: "生乾き臭（換気不足・配管問題の可能性）。清潔感評価に直結", count: "3件", color: "EA580C" },
  { pri: "B", cat: "防音・清掃騒音", detail: "朝9時頃の清掃作業音がゲストの休息を妨害", count: "1件", color: "D4A843" },
  { pri: "B", cat: "設備老朽化", detail: "外観に比べて客室が古いという印象。内装リフレッシュの余地あり", count: "3件", color: "D4A843" },
  { pri: "B", cat: "駐車場制限", detail: "高さのある車が入れない駐車場の制限への指摘", count: "1件", color: "D4A843" },
  { pri: "C", cat: "朝食メニュー", detail: "トースター未設置・魚種への好みの相違など細かな指摘", count: "1件", color: "94A3B8" },
];

// Slide 7: Phase 1
const PHASE1_CARDS = [
  { title: "スタッフ接客品質の均質化", items: ["接客マニュアル見直し・定期トレーニング", "フロント応対品質の月次チェック", "チェックアウト時満足度アンケート実施", "問題スタッフへの個別指導・フォロー"] },
  { title: "Google口コミ対策", items: ["全Google口コミへ48h以内返信", "満足ゲストへのGoogle口コミ案内", "低評価への真摯な公開返信", "Google評価の月次モニタリング"] },
  { title: "バスルーム衛生強化", items: ["清掃時の換気徹底（生乾き臭防止）", "排水口・換気扇の定期清掃強化", "バスルーム点検チェックリスト更新", "臭い発生客室の優先対応フロー整備"] },
  { title: "清掃作業の静粛化", items: ["朝9時以降の清掃作業音抑制ルール", "清掃スタッフへの静粛作業トレーニング", "清掃カートの静音化（タイヤ・扉）"] },
];

// Slide 8: Phase 2 & 3
const PHASE2_ITEMS = [
  { title: "内装リフレッシュ", items: ["老朽化客室のクロス・カーペット更新", "バスルーム設備の計画的更新", "共用部のリフレッシュ工事"] },
  { title: "朝食品質向上", items: ["トースター設置による朝食体験改善", "メニューの多様性強化（和洋選択肢）", "朝食時間帯の混雑緩和策"] },
  { title: "デジタル化推進", items: ["週次サイト別評価モニタリング体制確立", "口コミ返信の自動リマインダー導入"] },
];

const PHASE3_ITEMS = [
  { title: "バスルーム全面改修", items: ["換気システム改善による臭い根本解消", "シャワールームのリノベーション", "アメニティグレードアップ"] },
  { title: "客室グレードアップ", items: ["外観・内装の質感統一", "全室Wi-Fi高速化", "スマートルーム化（IoTデバイス導入）"] },
  { title: "サービス差別化", items: ["三大強み（立地・清潔感・スタッフ）を訴求した戦略的プロモーション", "Trip.com高評価をインバウンド集客に活用"] },
];
...
const KPI_NOTE_TEXT = "毎月末に全サイトのレビューデータを集計し、特にGoogle評価の推移を重点モニタリングする。スタッフ対応クレームは翌日以内に管理職エスカレーション。バスルームクレームはゼロ件達成まで週次点検を継続。";

// Slide 10: 総括
const CLOSING_PARAGRAPHS = [
  { text: "ハートンホテル京都は全体平均8.59点・高評価率82.9%と安定した評価を誇ります。立地・アクセス（76件）を筆頭に、客室設備・清潔感が三本柱として高い顧客満足度を支えており、特にTrip.com（9.43点）での卓越した評価は国際競争力を示しています。", options: { breakLine: true, fontSize: 12 } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "Google評価の低さ（6.8点・要改善）が最優先課題であり、スタッフ接客の均質化とバスルーム衛生改善が評価底上げの鍵です。これら改善施策は投資不要のオペレーション改善から着手可能であり、早期に成果が期待されます。", options: { breakLine: true, fontSize: 12 } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "じゃらん（8.63点）・楽天トラベル（8.58点）での安定した高評価を維持しながら、Google評価の改善とスタッフ品質の均質化を並行推進することで、全体平均8.8点以上・高評価率85%以上の達成が現実的な目標です。", options: { breakLine: true, fontSize: 12 } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "Google評価を3.4点から3.8/5点以上へ引き上げることが最重要KPIです。スタッフ接客品質の均質化とバスルーム衛生改善を軸に、顧客体験の底上げを図ってください。", options: { bold: true, fontSize: 12, color: "D4A843" } },
];
