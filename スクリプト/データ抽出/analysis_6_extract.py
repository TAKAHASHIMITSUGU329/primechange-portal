...
for h in hotels_ranked:
    ...
    hotels.append({
        ...
        "may_revenue": r.get("may_revenue", 0),
        "may_occupancy": r.get("may_occupancy", 0),
...
output = {
    ...
    "portfolio_summary": {
        ...
        "may_total_revenue": round(sum(h["may_revenue"] for h in hotels)),
        "may_avg_occupancy": f"{float(np.mean([h['may_occupancy'] for h in hotels]))*100:.1f}%",
    },
    "regression_results": {
