// Date Filter Engine V2 - PRIMECHANGE Portal
// Same as V1 with minor enhancements
(function() {
  'use strict';
  ...
  function calcHotelKPI(reviews) {
    ...
    var siteStats = Object.keys(siteMap).map(function(k) {
      ...
      return { site: s.site, count: s.count, avg_10pt: sAvg, judgment: getTier(sAvg) };
    ...
    return {
      total_reviews: total, overall_avg_10pt: avg,
      ...
      site_stats: siteStats, distribution: distribution, tier: tier
    };
  }
  ...
  function calcPortfolioKPI(allFiltered) {
    ...
    return {
      total_hotels: hotelKeys.length, total_reviews: totalReviews, avg_score: avgScore,
  ...
  function getDailyStats(reviews) {
    ...
    return days.map(function(d) {
      return { date: d, count: dayMap[d].count, avg: dayMap[d].count > 0 ? Math.round(dayMap[d].sum / dayMap[d].count * 100) / 100 : 0 };
    });
  }
  ...
  function movingAvg(dailyStats, window) {
    ...
    return dailyStats.map(function(d, i) {
      ...
      return { date: d.date, count: d.count, avg: d.avg, ma: count > 0 ? Math.round(sum / count * 100) / 100 : 0 };
    });
  }
  ...
  function applyFilter(start, end) {
    ...
    var event = new CustomEvent('dateFilterChanged', {
      detail: {
        range: currentRange, filteredCount: allFiltered.length,
        portfolio: portfolio, dailyStats: dailyMA, hotelDaily: hotelDaily,
        filteredReviews: filtered, cleaningKeywords: CLEANING_KEYWORDS,
        tierColor: TIER_COLOR, snapshotId: activeSnapshotId, snapshots: snapshotIndex
      }
    });
  ...
  function loadSnapshot(snapshotId) {
    if (!snapshotId) {
      if (latestReviews) {
        ...
        document.dispatchEvent(new CustomEvent('snapshotChanged', { detail: { snapshotId: null, isLatest: true } }));
      }
    ...
    return Promise.all([
      ...
      allReviews = results[0];
      if (results[1]) buildMeta = results[1];
      activeSnapshotId = snapshotId;
      applyFilter(null, null);
      document.dispatchEvent(new CustomEvent('snapshotChanged', { detail: { snapshotId: snapshotId, isLatest: false, meta: results[1] } }));
    }).catch(function(e) {
  ...
  function loadAllReviews() {
    ...
    if (window.__REVIEWS_DATA__) {
      allReviews = window.__REVIEWS_DATA__;
      latestReviews = window.__REVIEWS_DATA__;
      return Promise.resolve(allReviews);
    }
    return fetch('data/hotel-reviews-all.json')
      .then(function(r) { return r.json(); })
      .then(function(data) { allReviews = data; latestReviews = data; return data; })
      .catch(function(e) { console.warn('hotel-reviews-all.json load failed:', e); allReviews = {}; latestReviews = {}; return {}; });
  }

  function loadBuildMeta() {
    if (window.__BUILD_META__) {
      buildMeta = window.__BUILD_META__;
      latestMeta = window.__BUILD_META__;
      return Promise.resolve(buildMeta);
    }
    return fetch('data/build-meta.json')
      .then(function(r) { return r.json(); })
      .then(function(data) { buildMeta = data; latestMeta = data; return data; })
      .catch(function() { return null; });
  }

  function loadSnapshotIndex() {
    if (window.__SNAPSHOT_INDEX__) {
      snapshotIndex = window.__SNAPSHOT_INDEX__;
      return Promise.resolve(snapshotIndex);
    }
    ...
    Promise.all([loadAllReviews(), loadBuildMeta(), loadSnapshotIndex()])
      .then(function() {
        ...
        document.dispatchEvent(new CustomEvent('dateFilterReady', {
          detail: { buildMeta: buildMeta, snapshots: snapshotIndex, dataRange: buildMeta ? buildMeta.data_range : null }
        }));
      });
  }
  ...
  window.DateFilter = {
    apply: applyFilter, loadSnapshot: loadSnapshot,
    ...
    calcHotelKPI: calcHotelKPI, getDailyStats: getDailyStats, movingAvg: movingAvg,
    hasCleaningIssue: hasCleaningIssue, CLEANING_KEYWORDS: CLEANING_KEYWORDS,
    TIER_COLOR: TIER_COLOR, getTier: getTier
  };
})();
