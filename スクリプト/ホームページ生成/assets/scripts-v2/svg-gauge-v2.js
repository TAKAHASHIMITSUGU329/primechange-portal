// SVG Gauge Chart V2 - PRIMECHANGE Portal
...
(function() {
  ...
  function createSVGGauge(container, value, target, options) {
    ...
    var achievementPct;
    if (lower) {
      achievementPct = value <= 0 ? 100 : Math.round(Math.min(target / value, 1) * 1000) / 10;
      if (value <= target) achievementPct = 100;
    } else {
      achievementPct = target > 0 ? Math.round(value / target * 1000) / 10 : 0;
    }
    ...
    var svg = [
      ...
      '    ' + value + unit,
      '  </text>',
      '  <text x="' + cx + '" y="' + (cy + size * 0.15) + '" text-anchor="middle"',
      '    font-size="' + (size * 0.09) + '" fill="#94A3B8">',
      '    目標: ' + target + unit,
      '  </text>',
      '  <text x="' + cx + '" y="' + (cy + size * 0.26) + '" text-anchor="middle"',
      '    font-size="' + (size * 0.08) + '" font-weight="700" fill="' + statusColor + '">',
      '    達成率: ' + achievementPct + '%',
      '  </text>',
      '</svg>'
    ].join('\n');
