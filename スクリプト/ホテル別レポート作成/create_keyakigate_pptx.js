...
const SUMMARY_STRENGTHS = [
  { title: "駅直結の立地（35件以上）", desc: "府中駅から直結、雨でも濡れずアクセス可能" },
  { title: "朝食の高品質（20件以上）", desc: "一品の質が高く、高級ホテルを凌ぐとの評価" },
  { title: "清潔で新しい施設（15件以上）", desc: "築浅で清潔感があり安心して滞在可能" },
];

const SUMMARY_WEAKNESSES = [
  { title: "清掃品質のばらつき（4件）", desc: "シーツの毛髪・ゴミ、バスルーム清掃漏れ" },
  { title: "スタッフ連携不足（3件）", desc: "申し送り漏れ、同じ質問を繰り返される" },
  { title: "室内環境の課題（2件）", desc: "空調の効き不足、冬季の乾燥問題" },
];

const SITE_CHART_LABELS = ["Trip.com", "楽天トラベル", "Agoda", "じゃらん", "Booking.com", "Google"];
const SITE_CHART_VALUES = [10.0, 8.86, 8.83, 8.75, 8.70, 6.0];

const SITE_INSIGHT_TEXTS = [
  { text: "サイト間の評価傾向", options: { bold: true, fontSize: 12, breakLine: true } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "高評価グループ (8.7-10.0)", options: { bold: true, color: "16A34A", breakLine: true } },
  { text: "Trip.com/楽天/Agoda/じゃらん/Booking.comの5サイトで8.7点以上の良好な評価", options: { fontSize: 9, color: "64748B", breakLine: true } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "要注意 (6.0)", options: { bold: true, color: "DC2626", breakLine: true } },
  { text: "Google評価が6.0点と大幅に低い。件数3件と少数だが低評価が含まれ、改善が急務", options: { fontSize: 9, color: "64748B", breakLine: true } },
  { text: "", options: { fontSize: 6, breakLine: true } },
...
const DIST_SUMMARY_CARDS = [
  ...
  { label: "低評価 1-4点", val: "3件（4.5%）", col: "DC2626", bg: "FEE2E2" },
];

const STRENGTHS_CARDS = [
  { theme: "立地・アクセス", count: "35件+", desc: "駅直結で雨に濡れない利便性が圧倒的な評価。周辺の飲食店・商業施設も充実", quote: "「駅直結で徒歩1分、雨に濡れる心配もありません」" },
  { theme: "朝食の質", count: "20件+", desc: "品数は多くないが一品の質が高い。和食メニューが特に好評", quote: "「高級ホテルをも凌ぐクオリティ」" },
  { theme: "清潔感・新しさ", count: "15件+", desc: "築浅で清潔な施設。部屋も綺麗で安心して過ごせる", quote: "「新しくて清潔、駅近でとても満足です」" },
  { theme: "スタッフ対応", count: "8件", desc: "フロントの丁寧な対応、子供用セットの心遣いなど好印象", quote: "「きめ細やかなサービスが、さりげなくて良い」" },
  { theme: "部屋の快適さ", count: "10件", desc: "広めの部屋、ゆったりしたベッド、足が伸ばせるバスタブ", quote: "「部屋が広々してゆっくりできました」" },
  { theme: "リピート意向", count: "30件+", desc: "多くのゲストが再訪を希望。コスパの良さも評価", quote: "「この周辺で1番おすすめのホテルです」" },
];

const WEAKNESS_ITEMS = [
  { pri: "S", cat: "清掃品質のばらつき", detail: "シーツの毛髪・ゴミ残り、バスルーム清掃漏れ、部屋着の匂い", count: "4件", color: "DC2626" },
  { pri: "A", cat: "スタッフ情報連携不足", detail: "連泊時の申し送り漏れ、アメニティ依頼の未対応", count: "3件", color: "EA580C" },
  { pri: "A", cat: "フロント対応の温度差", detail: "スタッフにより接客品質にばらつき（愛想なし等）", count: "2件", color: "EA580C" },
  { pri: "B", cat: "空調・室内環境", detail: "エアコンの効き不足、冬季の乾燥（加湿器未設置）", count: "2件", color: "D4A843" },
  { pri: "B", cat: "Wi-Fi接続品質", detail: "2.4GHz帯のみで夕方〜夜に頻繁切断", count: "2件", color: "D4A843" },
  { pri: "B", cat: "眺望情報の未開示", detail: "すりガラス窓の部屋を予約時に説明なし", count: "1件", color: "D4A843" },
  { pri: "C", cat: "駐車場の不在", detail: "提携駐車場なく自力で探す必要あり", count: "2件", color: "94A3B8" },
  { pri: "C", cat: "共用設備の混雑", detail: "合宿利用者の洗濯機占有、ロビー居眠り", count: "2件", color: "94A3B8" },
];

const PHASE1_CARDS = [
  { title: "(1) 清掃チェック体制強化", items: ["チェックリスト導入（シーツ・バス・備品）", "ダブルチェック制度（1日3室以上）", "部屋着の嗅覚チェック追加", "口コミ指摘の即日フィードバック"] },
  { title: "(2) スタッフ情報連携改善", items: ["PMS記録の徹底と全員参照体制", "シフト交代時の引き継ぎチェックリスト", "アメニティ依頼のステータス管理"] },
  { title: "(3) フロント接客の標準化", items: ["到着時の第一声・笑顔の統一基準", "早着ゲスト対応マニュアル整備", "月1回のロールプレイング研修"] },
  { title: "(4) 客室情報の事前開示", items: ["眺望なし客室の予約サイト明記", "該当客室の写真追加掲載", "予約確認メールでの客室特徴案内"] },
];

const PHASE2_ITEMS_PPTX = [
  { title: "(1) Wi-Fi環境の改善", items: ["5GHz帯AP増設とチャネル最適化", "キャスト用とWi-Fiの帯域分離", "夕方〜夜間帯の定期接続テスト"] },
  { title: "(2) 客室環境の改善", items: ["加湿器の常設配備", "空調メンテナンス実施"] },
  { title: "(3) Google口コミ対策", items: ["チェックアウト時QRコード投稿依頼", "Google口コミ100%返信（48h以内）"] },
];

const PHASE3_ITEMS_PPTX = [
  { title: "(1) 駐車場ソリューション", items: ["近隣パーキングとの提携契約", "予約サイトでの駐車場情報充実", "駐車場予約代行の導入検討"] },
  { title: "(2) 朝食メニュー充実", items: ["季節限定メニューのローテーション強化", "洋食品目（ベーコン等）の追加"] },
  { title: "(3) 共用スペースルール整備", items: ["ランドリー利用時間制限・予約制", "ロビー利用マナー掲示と巡回強化"] },
];
...
const KPI_NOTE_TEXT = "月次で各サイトの口コミデータを収集・分析し、四半期ごとにKPI達成状況をレビュー。清掃関連クレームはゼロ件を目標に、即日対応・即日改善を徹底します。";

const CLOSING_PARAGRAPHS = [
  { text: "ホテルケヤキゲート東京府中は、駅直結の立地、高品質な朝食、清潔な施設を武器に全体平均8.74点の高評価を獲得しています。", options: { breakLine: true, fontSize: 12 } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "最優先課題の清掃品質安定化、スタッフ連携改善、Wi-Fi環境の整備を段階的に実施することで、高評価率90%以上の達成が見込めます。", options: { breakLine: true, fontSize: 12 } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "特にGoogle評価の改善は、新規ゲスト獲得に直結する重要施策です。全サイトでの口コミ返信100%を目標に、ゲストとの信頼関係を強化していきます。", options: { breakLine: true, fontSize: 12 } },
  { text: "", options: { fontSize: 6, breakLine: true } },
  { text: "「立地」「朝食」「清潔感」の三大強みを磨き上げ、府中エリアNo.1ホテルのポジションを確立しましょう。", options: { bold: true, fontSize: 12, color: "D4A843" } },
];
...
s3.addChart(pres.charts.BAR, [{ name: "10pt換算平均", labels: SITE_CHART_LABELS, values: SITE_CHART_VALUES }], {
  x: 0.5, y: 1.1, w: 5.5, h: 3.5, barDir: "bar", chartColors: [C.blue],
  chartArea: { fill: { color: C.white }, roundedCorners: true },
  catAxisLabelColor: C.grayDark, catAxisLabelFontSize: 10,
  valAxisLabelColor: C.gray, valAxisLabelFontSize: 9,
  valGridLine: { color: "E2E8F0", size: 0.5 }, catGridLine: { style: "none" },
  showValue: true, dataLabelPosition: "outEnd", dataLabelColor: C.grayDark, showLegend: false, valAxisMaxVal: 10,
});
...
s3.addTable([tblHeader2, ...tblRows], {
  x: 0.5, y: 4.7, w: 9, h: 0.1, colW: [2.0, 1.0, 2.0, 1.0, 3.0],
  border: { pt: 0.5, color: "CBD5E1" }, rowH: [0.25, 0.22, 0.22, 0.22, 0.22, 0.22, 0.22],
});
addFooter(s3, 3);
...
s4.addChart(pres.charts.DOUGHNUT, [{
  name: "評価分布（10pt換算）", labels: DIST_DOUGHNUT_LABELS, values: DIST_DOUGHNUT_VALUES,
}], {
  x: 0.3, y: 1.2, w: 4.0, h: 3.2, chartColors: [C.green, C.gold, C.red],
  showPercent: true, dataLabelColor: C.white, dataLabelFontSize: 11, showTitle: false,
  showLegend: true, legendPos: "b", legendFontSize: 9, legendColor: C.gray,
});
s4.addChart(pres.charts.BAR, [{
  name: "件数", labels: DIST_BAR_LABELS, values: DIST_BAR_VALUES,
}], {
  ...
  valGridLine: { color: "E2E8F0", size: 0.5 }, catGridLine: { style: "none" },
  showValue: true, dataLabelPosition: "outEnd", dataLabelColor: C.grayDark, dataLabelFontSize: 9, showLegend: false,
});
...
STRENGTHS_CARDS.forEach((s, i) => {
  ...
  const x = 0.5 + col * 3.1; const y = 1.15 + row * 2.0;
...
s6.addTable([wTblHeader, ...wTblRows], {
  ...
  border: { pt: 0.5, color: "CBD5E1" }, rowH: [0.3, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35],
});
s6.addText([
...
PHASE1_CARDS.forEach((p, i) => {
  ...
  const x = 0.5 + col * 4.6; const y = 1.35 + row * 1.95;
...
s9.addTable([kpiHeader, ...kpiData], {
  ...
  border: { pt: 0.5, color: "CBD5E1" }, rowH: [0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35, 0.35],
});
